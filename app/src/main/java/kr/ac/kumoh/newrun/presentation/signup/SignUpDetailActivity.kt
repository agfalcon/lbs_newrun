...
class SignUpDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnSignUpComplete.setOnClickListener {
            if(binding.etName.text.toString().length < 1) {
                Toast.makeText(applicationContext, "이름을 작성 후 제출해주세요!", Toast.LENGTH_SHORT).show()
            } else if(binding.etNickname.text.toString().length < 1) {
                Toast.makeText(applicationContext, "닉네임을 작성 후 제출해주세요!", Toast.LENGTH_SHORT).show()
            } else if(binding.etKm.text.toString().length < 1) {
                Toast.makeText(applicationContext, "목표 KM를 작성 후 제출해주세요!", Toast.LENGTH_SHORT).show()
            } else {
                ...
                if(userEmail != "null"){
                    ...
                    CoroutineScope(
                        ...
                        val result = SignUpService().snsSignUp(
                            SocialSignUp(
                                email = userEmail,
                                userMame = name,
                                nickName = nickname,
